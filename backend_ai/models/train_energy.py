"""
Energy Forecasting Trainer — hybrid-style (physics baseline + ML residual),
frequency-aware horizon, delta target, persistence gating, clean CLI.

MIT License — please keep this header.

Quick start:
    python backend_ai/models/train_energy.py \
...
Outputs:
    - scaler.pkl, xgb.pkl, lgb.pkl, stacker.pkl (if stacked wins), metrics.json
    - predictions_{head}.csv, plots under outdir
    - production_head.txt (which head is shipped)
"""
...
class TrainConfig:
    ...
    # LGB - Tuned for better generalization
    ...
    lgb_num_leaves: int = 40       # WAS 31
    lgb_min_data_in_leaf: int = 8  # WAS 10
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for L in lag_rows:
        ...
        # 💡 IMPROVEMENT 1: Add features for the power delta (change since last step)
    ...
    # 💡 IMPROVEMENT 2: Add more cyclical features for stronger seasonality
...
def prepare_data(train_df: pd.DataFrame, test_df: pd.DataFrame, horizon_min: int
    ...
    # delta target: y(t+H) - y(t), frequency-aware H
    htr = _rows_for_minutes(train_df.index, horizon_min)
    hte = _rows_for_minutes(test_df.index, horizon_min)
    ytr = train_df["power_kw"].shift(-htr) - train_df["power_kw"]
    yte = test_df["power_kw"].shift(-hte) - test_df["power_kw"]

    Xtr, ytr = align_xy(Xtr, ytr)
    Xte, yte = align_xy(Xte, yte)
    return Xtr, Xte, ytr.rename("delta_kw"), yte.rename("delta_kw"), htr, hte
...
def train_lgb(X: pd.DataFrame, y: pd.Series, cfg: TrainConfig) -> lgb.LGBMRegressor:
    ...
    # 💡 IMPROVEMENT 3: Added regularization and min_child_samples
    model = lgb.LGBMRegressor(
        n_estimators=cfg.lgb_estimators,
        learning_rate=cfg.lgb_lr,
        num_leaves=cfg.lgb_num_leaves,
        min_child_samples=cfg.lgb_min_data_in_leaf, # Use min_child_samples for min_data_in_leaf
...
# ---------------------- Baseline & Evaluation ------------------------
def persistence_baseline(test_df: pd.DataFrame, y_delta_test: pd.Series, horizon_rows: int
                         ) -> Tuple[Dict[str, float], np.ndarray, np.ndarray]:
    # true future level
    y_true_level = (test_df["power_kw"].shift(-horizon_rows)).loc[y_delta_test.index]
...
def evaluate_heads(
    ...
    y_base_level: np.ndarray,
    y_delta_true: pd.Series,
    persist_rmse: float,
) -> Dict[str, Dict[str, float]]:
    # align target level: y(t+H) = y(t) + ΔH_true
    y_true_level = y_base_level + y_delta_true.values
...
def run(cfg: TrainConfig):
    ...
    # Train trees on delta
    log.info("Training XGBoost...")
    xgbm = train_xgb(Xtr_s, ytr, cfg)
    log.info("Training LightGBM...")
    lgbm = train_lgb(Xtr_s, ytr, cfg)

    # Test-time delta predictions
    pred_xgb_delta = xgbm.predict(Xte_s)
    pred_lgb_delta = lgbm.predict(Xte_s)

    # Convert delta → level using current level at t
    base_level = Xte["power_kw_cur"].values
    pred_xgb_level = base_level + pred_xgb_delta
    pred_lgb_level = base_level + pred_lgb_delta

    # Median and stacked blends
    pred_median_level = np.median(np.vstack([pred_xgb_level, pred_lgb_level]), axis=0)

    stack = Ridge(alpha=0.5, positive=True, random_state=cfg.random_state)
    # Fit stacker on a forward validation split (delta domain), then apply and convert to level
    # 💡 IMPROVEMENT 4: Ensure the stacker split is a forward-split for time series
    ...
    pred_stack_delta = stack.predict(np.c_[pred_xgb_delta, pred_lgb_delta])
    pred_stack_level = base_level + pred_stack_delta

    # Persistence baseline & Skill
    base_mets, y_true_level_aligned, y_persist = persistence_baseline(te.loc[Xte.index], yte, hte)
    log.info(f"PERSISTENCE — MAE {base_mets['mae']:.3f} | RMSE {base_mets['rmse']:.3f} | "
             f"R² {base_mets['r2']:.3f} | MAPE {base_mets['mape']:.2f}%")

    # Align heads to y_true_level_aligned (same index ordering)
    # Our Xte/yte arrays are already aligned; just ensure lengths match baseline slice:
    ...
    heads = {
        ...
        "persistence": y_persist,  # for plotting/report
    }
    y_base_level_eval = base_level[-keep:]
    y_delta_true_eval = yte.iloc[-keep:]

    results = evaluate_heads(heads, y_base_level_eval, y_delta_true_eval, persist_rmse=base_mets["rmse"])
    for k, m in results.items():
        log.info(f"{k.upper()} — MAE {m['mae']:.3f} | RMSE {m['rmse']:.3f} | R² {m['r2']:.3f} | "
                 f"MAPE {m['mape']:.2f}% | Skill {m['skill_vs_persist']:.3%}")

    # Choose production: best positive skill; else persistence
    ...
    if results[best]["skill_vs_persist"] <= 0.0:
        ...
        log.warning(f"No model beat persistence (best={best}, skill={results[best]['skill_vs_persist']:.3%}). "
                    f"Shipping persistence.")
    else:
...
def parse_args() -> TrainConfig:
    p = argparse.ArgumentParser(description="Energy forecasting trainer (hybrid-style).")
